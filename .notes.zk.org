...
Var in...
+ clojure but not in cljs
+ both
+ cljs but not clojure

[cljs.core] gets mapped to clojure.core meaning any vars found in the
input namespaces with the same name as a var found in the target
namespace get mapped to target ns/name var.
